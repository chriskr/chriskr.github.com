...
(function () {
  ...
  this.createTemplate = function (tmpl, namespace) {
    var ELE_NAME = 0;
    var ATTRS = 1;
    var ele = null;
    ...
    if (typeof elementName === 'string' && elementName !== TEXT_NODE_NAME) {
      i++;
      if (namespace || elementName.includes(':')) {
        var pos = elementName.indexOf(':');
        if (pos > -1) {
          namespace = NAMESPACES[elementName.slice(0, pos)];
          elementName = elementName.slice(pos + 1);
        }
        ele = document.createElementNS(namespace, elementName);
      } else {
    ...
    for (; i < tmpl.length; i++) {
      if (typeof tmpl[i] === 'string') {
        ...
        ele.appendChild(bragi.createTemplate(tmpl[i], namespace));
      }
    }
    return ele;
  };
  ...
  if (!Element.prototype.matchesSelector) {
    Element.prototype.matchesSelector = Element.prototype.webkitMatchesSelector ? Element.prototype.webkitMatchesSelector : function (selector) {
      if (this.parentNode) {
  ...
  EventHandler.prototype = new function () {
    ...
    EventHandler.unregister = function (type, name, handler, isCapturing) {
      ...
      if (handler_map && handler_map[name] === handler) handler_map[name] = null;
    };

    var handler_ = function handler_(handler_map, event) {
      var ele = event.target;
      while (ele && !event.defaultPrevented) {
        var name = ele.getAttribute(KEY);
        if (name && handler_map[name]) handler_map[name](event, ele);
