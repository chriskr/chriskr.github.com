...
if (String.prototype.repeat === undefined) {
  ...
  String.prototype.repeat = function (count) {
    ...
    var str = '' + this;
    count = +count; // eslint-disable-next-line
...
var cell = function cell(i) {
  return "<td><span>cell ".concat(i, "</span></td>");
};

var row = function row(n, start) {
  return "<tr>".concat(_toConsumableArray(range(n)).map(function (i) {
    return cell(i + start);
  }).join(''), "</tr>");
};

var table = function table(n, m) {
  return "<table><tbody>".concat(_toConsumableArray(range(n)).map(function (i) {
    return row(m, i * m);
  }).join(''), "</tbody></table>");
};

var boxProps = ['top', 'left', 'right', 'bottom', 'width', 'height'];

var scaleBox = function scaleBox(box, scale) {
  return boxProps.reduce(function (newBox, prop) {
    return newBox[prop] = box[prop] * scale, newBox;
  }, {});
};
...
var DELTA = 0.1;
var DRAG_START_DELTA = 15;
var MAX_SCALE = 2;

var getDistance = function getDistance(_ref, _ref2) {
  var x1 = _ref.x,
      y1 = _ref.y;
  var x2 = _ref2.x,
      y2 = _ref2.y;
...
function () {
  function ScaleManager() {
    ...
    _defineProperty(this, "_contaninerElement", null);

    _defineProperty(this, "_contentElement", null);

    _defineProperty(this, "_scale", 1);

    _defineProperty(this, "_translateX", 0);

    _defineProperty(this, "_translateY", 0);

    _defineProperty(this, "_dragStart", {
      deltaX: 0,
      deltaY: 0
    });
    ...
    _defineProperty(this, "_mousemove", function (event) {
      ...
      if (!_this._isDrag && getDistance({
        x: x,
        y: y
      }, {
        x: _this._dragStart.x,
        y: _this._dragStart.y
      }) < DRAG_START_DELTA) {
        return;
      }
  ...
  _createClass(ScaleManager, [{
    key: "setDragStartPoint",
    value: function setDragStartPoint(_ref3) {
      var x = _ref3.x,
          y = _ref3.y;
      this._dragStart = {
    ...
    value: function center() {
      var containerBox = this._contaninerElement.getBoundingClientRect();

      var contentBox = scaleBox(this._contentElement.getBoundingClientRect(), 1 / this._scale);
      this._scale = Math.min(containerBox.width / contentBox.width, containerBox.height / contentBox.height);
      var scaledBox = scaleBox(contentBox, this._scale);
    ...
    key: "scaleAt",
    value: function scaleAt(scale, _ref4) {
      var x = _ref4.x,
          y = _ref4.y;

      var containerBox = this._contaninerElement.getBoundingClientRect();

      var contentBox = this._contentElement.getBoundingClientRect();

      var contentBoxScaled = scaleBox(contentBox, 1 / this._scale);
      var minScale = Math.min(containerBox.width / contentBoxScaled.width, containerBox.height / contentBoxScaled.height);
      scale = clamp(scale, minScale, MAX_SCALE);
      var deltaX = x - containerBox.left;
      var deltaY = y - containerBox.top;
      var contentX = deltaX - this._translateX;
      var contentY = deltaY - this._translateY;
    ...
    value: function _update() {
      this._contentElement.style.transform = "translate(".concat(this._translateX, "px, ").concat(this._translateY, "px) scale(").concat(this._scale, ")");
    }
  }, {
    key: "_move",
    value: function _move(_ref5) {
      var x = _ref5.x,
          y = _ref5.y;
    ...
    key: "_setTranslate",
    value: function _setTranslate(_ref6) {
      var x = _ref6.x,
          y = _ref6.y;
      var containerBox = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this._contaninerElement.getBoundingClientRect();
      var contentBox = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : this._contentElement.getBoundingClientRect();
      var maxDeltaX = containerBox.width - contentBox.width;
      var maxDeltaY = containerBox.height - contentBox.height;
    ...
    key: "contaninerElement",
    set: function set(contaninerElement) {
      this._contaninerElement = contaninerElement;
    }
  }, {
    key: "contentElement",
    set: function set(contentElement) {
...
window.onload = function () {
  ...
  var contentElement = document.querySelector('#inner-container');
  contentElement.innerHTML = table(20, 20);
  contentElement.style.width = "".concat(contentElement.firstChild.offsetWidth, "px");
  var scaleManger = new ScaleManager();
  scaleManger.contaninerElement = containerElement;
  scaleManger.contentElement = contentElement;
  document.querySelector('#center-content').addEventListener('click', function () {
    return scaleManger.center();
  });
  scaleManger.center();
  containerElement.addEventListener('mousedown', function (_ref7) {
    var x = _ref7.clientX,
        y = _ref7.clientY;
    return scaleManger.setDragStartPoint({
      x: x,
      y: y
    });
  });
  document.addEventListener('wheel', function (event) {
    var scale = scaleManger.scale + (event.deltaY < 0 ? 1 : -1) * DELTA * scaleManger.scale;
